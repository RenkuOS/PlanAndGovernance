...
five of six entries read **Proposed** would be fiction. This proposes something narrower and
more useful.

The project's actual communications exposure is specific and already present:

1. **It was founded on a disagreement with an existing project.** Every sentence written
...
Every document below exists to close one of those four. If a proposed document cannot be
traced back to one of them, it should not be written.
...
| `BRAND.md` | Name, logotype, claim, the mascot question, and what third parties may do with the marks | — | **Before the repository opens** |
...
### 3.1 The three sentences

One paragraph the project agrees on and reuses verbatim: what RenkuOS is, who it is for,
and why it exists. Written once, quoted everywhere, changed by council decision only.
...
a framing at the moment they are most likely to invent a bad one.
...
[`REVIEW.md`](REVIEW.md) §0.1–§0.3 is the strongest writing in this repository and it is
...
- **Announcements** — releases, decisions, incidents — come from the council or a named
  communications owner. This is the fourth open owner question, alongside D2's.
- **Nobody speaks for the project about the other project.** §3.2 is the whole policy.
- **Security disclosures follow `SECURITY.md`**, not the forum, and not a blog post first.
...
simpler, and the sharpest objection came from a participant with thirty years in marketing
communications:
...
Worth recording because it is the strongest fact in the whole discussion and it is easy to
lose: **the dog is the Tracker icon**, carried from BeOS through Haiku. It is not a mascot
this project chose — it is the one mark its audience has recognised for twenty years, and
*Continue* names exactly that continuity.
...
- **The marks.** `assets/` holds three logo revisions with no statement of which is current.
  ...
  The logotype stays typographic — the part of the current work nobody objected to.
...
This last point may belong with the non-profit's formation. Flagging it as a dependency
